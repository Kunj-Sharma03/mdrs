...
const Masonry = ({
  ...
  const { layoutItems, layoutHeight } = useMemo(() => {
    if (!width || !items.length) {
      return { layoutItems: [], layoutHeight: 0 };
    }
    ...
    const positioned = items.map(child => {
      ...
      const height = (child.height ?? columnWidth) / 2;
    ...
    return {
      ...
      layoutHeight: maxHeight > 0 ? maxHeight : Math.max(...positioned.map(item => item.h), 0)
    };
  ...
  useLayoutEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [layoutItems, imagesReady, stagger, animateFrom, blurToFocus, duration, ease]);
